...
def _by_source_block(by_source: Dict[str, Any]) -> str:
    ...
    for stats in ordered:
        ...
        rows.append(
            ...
            f'<td class="ok">{stats.get("confirmed", 0)}</td>'
            f'<td class="bad">{stats.get("not_confirmed", 0)}</td>'
            f"<td>{stats.get('undecided', 0)}</td>"
            f"<td>{rate}</td>"
            f"<td>{stats.get('avg_claimed_confidence', 0):.2f}</td>"
            "</tr>"
        )
    rows.append("</table>")
    return "".join(rows)
